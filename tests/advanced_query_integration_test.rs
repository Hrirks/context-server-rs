...
use context_server_rs::services::advanced_query_service::{
    AdvancedQueryConfig, AdvancedQueryService, AdvancedQueryServiceImpl, AdvancedSearchFilters,
    AdvancedSearchQuery, QualityRange, RankingPreferences, SuggestionType, UserContext,
    UserPreferences,
};
use tokio;
...
async fn test_advanced_query_features_integration() {
    ...
    assert!(
        ...
        "Query suggestions should not be empty"
    );

    // Verify suggestion types
    let has_intent = suggestions
        .iter()
        .any(|s| s.suggestion_type == SuggestionType::Intent);
    assert!(has_intent, "Should have intent-based suggestions");

    // Test auto-completion
    let completions = service
        .auto_complete("database", 8, Some("project-1"))
        .await
        .unwrap();

    assert!(
        !completions.is_empty(),
        "Auto-completion should not be empty"
    );
    ...
    for completion in &completions {
        assert!(
            completion.len() > "database".len(),
            "Completion should be longer than partial query"
        );
        assert!(
            completion.contains("database"),
            "Completion should contain the partial query"
        );
    }
...
async fn test_cross_project_search_capabilities() {
    ...
    let accessible_projects = vec![
        "project-1".to_string(),
        "project-2".to_string(),
        "project-3".to_string(),
    ];
    let matches = service
        .cross_project_search("authentication", &accessible_projects, 5)
        .await
        .unwrap();
...
async fn test_advanced_search_filters() {
    ...
    let (filtered_results, stats) = service
        .apply_filters(mock_results.clone(), &quality_filters)
        .await
        .unwrap();
...
async fn test_advanced_search_with_user_context() {
    ...
    let result = service
        .advanced_search(&query, Some(&user_context))
        .await
        .unwrap();
    ...
    assert!(
        ...
        "Should have suggestions when requested"
    );
    assert!(
        result.search_metadata.total_search_time_ms > 0,
        "Should track search time"
    );
    assert!(
        !result.search_metadata.filters_applied.is_empty(),
...
async fn test_filter_suggestions() {
    ...
    let filter_suggestions = service
        .suggest_filters("authentication", &mock_results, Some("project-1"))
        .await
        .unwrap();

    assert!(
        !filter_suggestions.is_empty(),
        "Should provide filter suggestions"
    );

    // Should suggest date filter for recent results
    let date_suggestions: Vec<_> = filter_suggestions
        .iter()
    ...
    for suggestion in &filter_suggestions {
        assert!(
            !suggestion.filter_type.is_empty(),
            "Filter type should not be empty"
        );
        assert!(
            !suggestion.filter_value.is_empty(),
            "Filter value should not be empty"
        );
        assert!(
        ...
        assert!(
            suggestion.estimated_reduction >= 0.0 && suggestion.estimated_reduction <= 1.0,
            "Estimated reduction should be between 0 and 1"
        );
    }
...
async fn test_disabled_features() {
    ...
    let suggestions = service
        .suggest_queries("auth", Some("project-1"), &[])
        .await
        .unwrap();
    assert!(
        suggestions.is_empty(),
    ...
    assert!(
        completions.is_empty(),
        "Auto-completion should be empty when disabled"
    );
    ...
    assert!(
        matches.is_empty(),
        "Cross-project search should be empty when disabled"
    );
...
async fn test_search_performance_metadata() {
    ...
    assert!(
        metadata.total_search_time_ms > 0,
        "Should track total search time"
    );
    assert!(
        metadata.query_processing_time_ms > 0,
        "Should track query processing time"
    );
    assert!(
        !metadata.filters_applied.is_empty(),
        "Should track applied filters"
    );
    assert!(
        !metadata.search_strategy.is_empty(),
        "Should track search strategy"
    );
    ...
    assert_eq!(
        ...
        "Simplified implementation has no results"
    );
