...
impl SqliteEmbeddingRepository {
    ...
    fn row_to_embedding(&self, row: &Row) -> SqliteResult<ContextEmbedding> {
        ...
        let embedding_vector: Vec<f32> =
            serde_json::from_str(&embedding_vector_json).map_err(|e| {
        ...
        let preprocessing_steps: Vec<String> =
            serde_json::from_str(&preprocessing_steps_json).unwrap_or_default();
...
impl EmbeddingRepository for SqliteEmbeddingRepository {
    async fn store_embedding(
        ...
        let preprocessing_steps_json =
            serde_json::to_string(&embedding.metadata.preprocessing_steps)?;
    ...
    async fn get_embedding_by_context_id(
        ...
        let mut embedding_iter = stmt.query_map([context_id], |row| self.row_to_embedding(row))?;

        if let Some(embedding) = embedding_iter.next() {
    ...
    async fn get_embeddings_by_project(
        ...
        let embedding_iter = stmt.query_map([project_id], |row| self.row_to_embedding(row))?;
    ...
    async fn find_similar_embeddings(
        ...
        let query_embedding = query.query_embedding.as_ref().ok_or_else(|| {
            EmbeddingRepositoryError::InvalidVectorData {
    ...
    async fn delete_embeddings_by_project(
        ...
        info!(
            "Deleted {} embeddings for project: {}",
            rows_affected, project_id
        );
        Ok(())
    }
    ...
    async fn get_embedding_stats(
        ...
        let total_embeddings: u64 = if let Some(pid) = project_id {
            ...
            conn.query_row("SELECT COUNT(*) FROM context_embeddings", [], |row| {
                row.get::<_, i64>(0)
            })? as u64
        };
        ...
        // Get date range
        let (oldest_str, newest_str): (Option<String>, Option<String>) = if let Some(pid) =
            project_id
        {
            conn.query_row(
        ...
        let oldest_embedding = oldest_str
            .and_then(|s| DateTime::parse_from_rfc3339(&s).ok())
            .map(|dt| dt.with_timezone(&Utc));
        let newest_embedding = newest_str
            .and_then(|s| DateTime::parse_from_rfc3339(&s).ok())
            .map(|dt| dt.with_timezone(&Utc));
