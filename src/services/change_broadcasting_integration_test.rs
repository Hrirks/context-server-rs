...
async fn test_delta_calculation_in_updates() {
    ...
    assert!(
        received_change.delta.is_some(),
        "Delta should be calculated for updates"
    );
...
async fn test_metrics_collection() {
    ...
    let initial_broadcasts = initial_metrics
        .total_changes_broadcast
        .load(std::sync::atomic::Ordering::Relaxed);
    let updated_broadcasts = updated_metrics
        .total_changes_broadcast
        .load(std::sync::atomic::Ordering::Relaxed);

    assert!(
        updated_broadcasts > initial_broadcasts,
