...
use crate::services::semantic_search_service::{
    SearchIndexStats, SemanticSearchError, SemanticSearchService,
};
...
pub trait SearchIndexManager: Send + Sync {
    ...
    ) -> Result<IndexHealthReport, IndexManagerError>;

    /// Clean up stale embeddings
    async fn cleanup_stale_embeddings(
        &self,
        project_id: Option<&str>,
    ) -> Result<usize, IndexManagerError>;
...
impl SearchIndexManagerImpl {
    ...
    async fn is_embedding_current(
        &self,
        context: &EnhancedContextItem,
    ) -> Result<bool, IndexManagerError> {
        let embedding_exists = self
            .embedding_repository
    ...
    async fn process_single_operation(
        ...
        match operation.operation_type {
            ...
            IndexOperationType::Delete => {
                self.semantic_search_service
                    .remove_from_index(&operation.context_id)
                    .await?;
        ...
        if total_ops > 0 {
            stats.average_indexing_time_ms =
                (stats.average_indexing_time_ms * (total_ops - 1) as f64 + processing_time)
                    / total_ops as f64;
        } else {
    ...
    async fn calculate_health_score(
        &self,
        project_id: Option<&str>,
    ) -> Result<f32, IndexManagerError> {
        let index_stats = self
            .semantic_search_service
            .get_index_stats(project_id)
            .await?;
        ...
        let coverage_score = if index_stats.total_indexed_items > 0 {
            1.0
        } else {
            0.0
        };
    ...
    async fn generate_recommendations(&self, health_report: &IndexHealthReport) -> Vec<String> {
        ...
        if health_report.overall_health_score < 0.7 {
            recommendations
                .push("Consider running index optimization to improve performance".to_string());
        }
        ...
        if health_report.index_freshness_score < 0.8 {
            recommendations
                .push("Index freshness is low, consider more frequent updates".to_string());
        }

        if health_report.average_quality_score < 0.6 {
            recommendations
                .push("Average embedding quality is low, review content preprocessing".to_string());
        }

        if health_report.performance_metrics.average_search_time_ms > 500.0 {
            recommendations
                .push("Search performance is slow, consider index optimization".to_string());
        }
...
impl SearchIndexManager for SearchIndexManagerImpl {
    async fn auto_index_context(
        ...
        if context.quality_score < self.config.min_quality_threshold as f64 {
            debug!(
                "Context quality {} below threshold {}, skipping indexing",
        ...
        if self.is_embedding_current(context).await? {
            debug!(
                "Context {} already has current embedding, skipping",
                context.id
            );
            return Ok(());
        }
    ...
    async fn process_pending_operations(&self) -> Result<IndexOperationStats, IndexManagerError> {
        ...
        info!(
            "Processed {} operations successfully, {} failed",
    ...
    async fn optimize_index(&self, project_id: Option<&str>) -> Result<(), IndexManagerError> {
        ...
        info!(
            "Index optimization completed. Cleaned {} stale embeddings",
            cleaned_count
        );
    ...
    async fn get_health_report(
        ...
        debug!(
            "Generating index health report for project: {:?}",
            project_id
        );
        ...
        debug!(
            "Health report generated with score: {:.2}",
    ...
    async fn rebuild_project_index(
        ...
        self.semantic_search_service
            .rebuild_index(project_id, contexts)
            .await?;
...
mod tests {
    ...
    use crate::services::semantic_search_service::{EnhancedSearchResult, SemanticSearchService};
    use std::collections::HashMap;
    ...
    impl EmbeddingRepository for MockEmbeddingRepository {
        ...
        async fn store_embeddings_batch(
            &self,
            _embeddings: &[ContextEmbedding],
        ) -> Result<(), EmbeddingRepositoryError> {
            Ok(())
        }

        async fn get_embedding_by_context_id(
            &self,
            _context_id: &str,
        ) -> Result<Option<ContextEmbedding>, EmbeddingRepositoryError> {
            Ok(None)
        }

        async fn get_embeddings_by_project(
            &self,
            _project_id: &str,
        ) -> Result<Vec<ContextEmbedding>, EmbeddingRepositoryError> {
            Ok(Vec::new())
        }
        ...
        async fn find_similar_embeddings(
            ...
            _query: &VectorSearchQuery,
            _project_id: Option<&str>,
        ) -> Result<Vec<VectorSearchResult>, EmbeddingRepositoryError> {
            Ok(Vec::new())
        }
        ...
        async fn delete_embeddings_by_project(
            &self,
            _project_id: &str,
        ) -> Result<(), EmbeddingRepositoryError> {
            Ok(())
        ...
        async fn get_embedding_stats(
            ...
            crate::repositories::embedding_repository::EmbeddingStats,
    ...
    impl EmbeddingService for MockEmbeddingService {
        async fn generate_embedding(
            ...
            _text: &str,
            _content_type: &str,
        ) -> Result<ContextEmbedding, EmbeddingError> {
        ...
        async fn generate_embeddings_batch(
            &self,
            _texts: Vec<(&str, &str, &str)>,
        ) -> Result<Vec<ContextEmbedding>, EmbeddingError> {
            Ok(Vec::new())
        }

        fn calculate_similarity(
            &self,
            _embedding1: &ContextEmbedding,
            _embedding2: &ContextEmbedding,
        ) -> f32 {
            0.8
        }
        ...
        async fn find_similar(
            ...
            _query: &VectorSearchQuery,
            _embeddings: &[ContextEmbedding],
        ) -> Result<Vec<crate::models::embedding::VectorSearchResult>, EmbeddingError> {
            Ok(Vec::new())
        }
        ...
        async fn update_config(
            &mut self,
            _config: crate::models::embedding::EmbeddingConfig,
    ...
    async fn test_queue_for_indexing() {
        ...
        let context = create_test_context();
        let result = manager
            .queue_for_indexing(context, IndexPriority::High)
            .await;
    ...
    async fn test_process_pending_operations() {
        ...
        manager
            .queue_for_indexing(context1, IndexPriority::Normal)
            .await
            .unwrap();
        manager
            .queue_for_indexing(context2, IndexPriority::High)
            .await
            .unwrap();
    ...
    async fn test_get_health_report() {
        ...
        let report = manager
            .get_health_report(Some("test-project"))
            .await
            .unwrap();
