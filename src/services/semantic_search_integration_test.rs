...
use crate::models::enhanced_context::{
    ContextContent, ContextMetadata, ContextType, EnhancedContextItem,
};
...
use crate::services::hybrid_search_service::{
    HybridSearchConfig, HybridSearchService, HybridSearchServiceImpl,
};
use crate::services::semantic_search_service::{
    SemanticSearchConfig, SemanticSearchService, SemanticSearchServiceImpl,
};
...
impl SemanticSearchIntegrationTest {
    ...
    pub async fn new() -> Result<Self, Box<dyn std::error::Error + Send + Sync>> {
        ...
        let embedding_service =
            EmbeddingServiceFactory::create_initialized_service(embedding_config.clone()).await?;
        ...
        let embedding_service_for_hybrid =
            EmbeddingServiceFactory::create_initialized_service(embedding_config.clone()).await?;
    ...
    pub async fn test_natural_language_query_processing(
        &self,
        ...
        let test_queries = vec![
            (
                ...
                "Should detect FindImplementation intent",
            ),
            (
                "best practices for error handling",
                "Should detect FindBestPractices intent",
            ),
            (
                "show me examples of REST API",
                "Should detect FindExamples intent",
            ),
            (
                "architecture for microservices",
                "Should detect FindArchitecture intent",
            ),
            (
                "security considerations for user data",
                "Should detect FindSecurity intent",
            ),
            (
                "performance optimization techniques",
                "Should detect FindPerformance intent",
            ),
            (
                "documentation about the system",
                "Should detect FindDocumentation intent",
            ),
        ];
        ...
        for (query_text, description) in test_queries {
            ...
            debug!(
                "Query '{}' returned {} results with intent-based ranking",
                query_text,
                results.len()
            );
        }
    ...
    pub async fn test_enhanced_result_ranking(
        &self,
    ) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        assert!(
            !results.is_empty(),
            "Enhanced ranking query should return results"
        );

        // Check that results are properly ranked (similarity scores should be in descending order)
        for i in 1..results.len() {
            assert!(
                results[i - 1].vector_result.similarity_score
                    >= results[i].vector_result.similarity_score,
        ...
        debug!(
            "Enhanced ranking returned {} results with proper ordering",
            results.len()
        );
    ...
    pub async fn test_hybrid_search_integration(
        &self,
    ) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        let suggestions = self
            .hybrid_search_service
            .get_search_suggestions("auth", Some("test-project"))
            .await?;

        assert!(
            !suggestions.is_empty(),
            "Search suggestions should be generated"
        );
        assert!(
        ...
        debug!(
            "Hybrid search executed successfully with strategy: {:?}",
            hybrid_result.search_strategy
        );
        debug!(
            "Combined score: {:.3}, Total results: {}",
