...
async fn test_subscription_update() {
    ...
    broadcaster
        .subscribe(client_id, initial_filters)
        .await
        .unwrap();

    // Update subscription
    broadcaster
        .update_subscription(client_id, updated_filters.clone())
        .await
        .unwrap();
    ...
    let result = broadcaster
        .update_subscription(non_existent_client, updated_filters)
        .await;
...
async fn test_delta_calculation() {
    ...
    let changed_fields = delta_value
        .get("changed_fields")
        .unwrap()
        .as_array()
        .unwrap();
...
async fn test_change_queue_management() {
    ...
    broadcaster
        .subscribe(client_id, vec![SyncFilters::default()])
        .await
        .unwrap();
    ...
    broadcaster
        .acknowledge_change(client_id, change.change_id)
        .await
        .unwrap();
...
async fn test_change_receiver() {
    ...
    tokio::spawn(async move {
        ...
        broadcaster_clone
            .broadcast_change(change_event)
            .await
            .unwrap();
    });

    // Receive the change
    let received_change = tokio::time::timeout(std::time::Duration::from_secs(1), receiver.recv())
        .await
        .unwrap()
        .unwrap();
