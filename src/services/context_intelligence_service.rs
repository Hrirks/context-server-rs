...
use crate::services::{
    ContextQualityService, ContextRelationshipEngine, DefaultContextQualityService,
    DefaultContextRelationshipEngine,
};
use anyhow::Result;
...
pub trait ContextIntelligenceService: Send + Sync {
    ...
    async fn recommend_context_improvements(
        &self,
        contexts: &[EnhancedContextItem],
    ) -> Result<Vec<ContextRecommendation>>;

    /// Analyze context gaps and suggest missing information
    async fn analyze_context_gaps(
        &self,
        project_contexts: &[EnhancedContextItem],
...
impl ContextIntelligenceService for DefaultContextIntelligenceService {
    async fn analyze_context(
        ...
        let quality_metrics = self
            .quality_service
            .calculate_quality_metrics(context, all_contexts)
            .await?;

        // Get validation results
        let validation_rules = self
            .quality_service
            .get_validation_rules(&context.content.content_type);
        let validation_result = self
            .quality_service
            .validate_context(context, &validation_rules)
            .await?;

        // Get improvement suggestions
        let improvement_suggestions = self
            .quality_service
            .generate_improvement_suggestions(context, &quality_metrics)
            .await?;

        // Detect relationships
        let relationships = self
            .relationship_engine
            .detect_relationships(context, all_contexts)
            .await?;

        // Find related contexts
        let related_contexts = self
            .relationship_engine
            .find_related_contexts(&context.id, all_contexts, 3)
            .await?;

        // Calculate intelligence score
        let intelligence_score =
            calculate_intelligence_score(&quality_metrics, &validation_result, &relationships);
        ...
        for context in contexts {
            ...
            if intelligence.intelligence_score < 0.7 {
                let recommendation = ContextRecommendation {
                    ...
                    description: format!(
                        "Context has low intelligence score ({:.2})",
                        intelligence.intelligence_score
                    ),
                    priority: if intelligence.intelligence_score < 0.5 {
                        Priority::High
                    } else {
                        Priority::Medium
                    },
            ...
            if intelligence.detected_relationships.is_empty() && contexts.len() > 1 {
                let recommendation = ContextRecommendation {
                    ...
                    actions: vec![
                        "Review related contexts and establish appropriate relationships"
                            .to_string(),
                    ],
                    created_at: Utc::now(),
                };
    ...
    async fn generate_project_insights(
        ...
        let project_contexts: Vec<_> = contexts
            .iter()
        ...
        for context in &project_contexts {
            *type_distribution
                .entry(context.content.content_type.clone())
                .or_insert(0) += 1;
        }
        ...
        for context in &project_contexts {
            let quality_metrics = self
                .quality_service
                .calculate_quality_metrics(context, &project_contexts)
                .await?;
        ...
        quality_contexts.sort_by(|a, b| {
            b.1.overall_score
                .partial_cmp(&a.1.overall_score)
                .unwrap_or(std::cmp::Ordering::Equal)
        });

        let top_quality_contexts = quality_contexts
            .iter()
            .take(5)
        ...
        let low_quality_contexts = quality_contexts
            .iter()
            .rev()
            .take(5)
        ...
        let actual_relationships: usize =
            project_contexts.iter().map(|c| c.relationships.len()).sum();

        let relationship_density =
            actual_relationships as f64 / total_possible_relationships as f64;

        // Analyze gaps and get recommendations
        let context_gaps = self.analyze_context_gaps(&project_contexts).await?;
        let recommendations = self
            .recommend_context_improvements(&project_contexts)
            .await?;
...
fn calculate_intelligence_score(
    ...
    let relationship_score = if relationships.is_empty() {
        ...
        let avg_strength: f64 =
            relationships.iter().map(|r| r.strength).sum::<f64>() / relationships.len() as f64;
    ...
    async fn generate_suggestions(
        ...
        for context in contexts {
            ...
            let title_similarity =
                calculate_text_similarity(&query.query_text, &context.content.title);
            let desc_similarity =
                calculate_text_similarity(&query.query_text, &context.content.description);
            let relevance_score = (title_similarity * 0.6 + desc_similarity * 0.4)
                .max(0.0)
                .min(1.0);
...
impl GapAnalyzer for TypeCoverageAnalyzer {
    async fn analyze_gaps(&self, contexts: &[EnhancedContextItem]) -> Result<Vec<ContextGap>> {
        ...
        let existing_types: HashSet<_> = contexts.iter().map(|c| &c.content.content_type).collect();
            ...
            if !existing_types.contains(&expected_type) {
                gaps.push(ContextGap {
                    ...
                    suggested_actions: vec![format!(
                        "Add {} context items to improve project coverage",
...
impl GapAnalyzer for RelationshipGapAnalyzer {
    async fn analyze_gaps(&self, contexts: &[EnhancedContextItem]) -> Result<Vec<ContextGap>> {
        ...
        let isolated_contexts: Vec<_> = contexts
            .iter()
        ...
        if !isolated_contexts.is_empty() && contexts.len() > 1 {
            gaps.push(ContextGap {
                ...
                description: format!(
                    "{} contexts have no relationships to other contexts",
                    isolated_contexts.len()
                ),
                severity: 0.6,
                impact_score: 0.5,
...
impl GapAnalyzer for QualityGapAnalyzer {
    async fn analyze_gaps(&self, contexts: &[EnhancedContextItem]) -> Result<Vec<ContextGap>> {
        ...
        let low_quality_contexts: Vec<_> =
            contexts.iter().filter(|c| c.quality_score < 0.5).collect();
        ...
        if !low_quality_contexts.is_empty() {
            gaps.push(ContextGap {
                ...
                description: format!(
                    "{} contexts have quality scores below 0.5",
                    low_quality_contexts.len()
                ),
                severity: 0.8,
                impact_score: 0.7,
                suggested_actions: vec![
...
mod tests {
    ...
    async fn test_analyze_context() {
        ...
        let intelligence = service
            .analyze_context(&context, &all_contexts)
            .await
            .unwrap();
    ...
    async fn test_suggest_related_contexts() {
        ...
        let suggestions = service
            .suggest_related_contexts(&query, &contexts)
            .await
            .unwrap();
    ...
    async fn test_recommend_context_improvements() {
        ...
        let recommendations = service
            .recommend_context_improvements(&contexts)
            .await
            .unwrap();
    ...
    async fn test_analyze_context_gaps() {
        ...
        assert!(gaps
            .iter()
            .any(|g| g.gap_type == GapType::MissingContextType));
    }
    ...
    async fn test_generate_project_insights() {
        ...
        let insights = service
            .generate_project_insights("project1", &contexts)
            .await
            .unwrap();
        ...
        assert!(insights
            ...
            .contains_key(&ContextType::BusinessRule));
        assert!(insights
            .context_type_distribution
            .contains_key(&ContextType::SecurityPolicy));
    }
