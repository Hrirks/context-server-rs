...
pub trait AnalyticsService: Send + Sync {
    ...
    async fn get_entity_usage(&self, entity_type: &str, entity_id: &str)
...
impl AnalyticsService for DefaultAnalyticsService {
    ...
    async fn get_project_insights(&self, project_id: &str) -> Result<ProjectInsights> {
        ...
        let total_duration: u64 = events.iter().filter_map(|e| e.duration_ms).sum();
        let events_with_duration = events.iter().filter(|e| e.duration_ms.is_some()).count();
        ...
        let mut most_active_entity_types: Vec<(String, u64)> =
            entity_type_counts.into_iter().collect();
        ...
        if success_rate < 0.9 {
            recommendations
                .push("Consider reviewing error patterns to improve success rate".to_string());
        }
        if average_response_time_ms > 1000.0 {
            recommendations
                .push("Response times are high, consider optimizing queries".to_string());
        }
        if total_events < 10 {
            recommendations
                .push("Low usage detected, consider promoting context server features".to_string());
        }
