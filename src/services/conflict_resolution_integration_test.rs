...
use crate::services::conflict_resolution_engine::{
    ConflictResolutionEngine, ConflictType, ManualResolutionRequest,
};
use crate::services::conflict_resolution_ui::{
    ConflictResolutionStep, ConflictResolutionUI, StartResolutionRequest, UpdateUIStateRequest,
};
use crate::services::sync_engine::SyncEngine;
use crate::services::websocket_types::{
    ChangeMetadata, ChangeType, ClientId, ConflictStrategy, ContextChange,
};
use anyhow::Result;
use chrono::Utc;
use serde_json::json;
...
async fn test_complete_conflict_resolution_workflow() -> Result<()> {
    ...
    assert_eq!(
        start_response.recommended_strategy,
    ...
    assert_eq!(
        resolution_result.strategy_used,
        ConflictStrategy::LastWriterWins
    );
...
async fn test_conflict_cleanup_and_session_management() -> Result<()> {
    ...
    conflict_ui
        .cancel_resolution(&start_response.session_id)
        .await?;

    let cancelled_session = conflict_ui.get_session(&start_response.session_id);
    assert!(cancelled_session.is_some());
    assert_eq!(
        cancelled_session.unwrap().ui_state.current_step,
        ConflictResolutionStep::Cancelled
    );
