...
use crate::models::enhanced_context::{ContextContent, ContextType, EnhancedContextItem};
use crate::repositories::embedding_repository::{
    EmbeddingRepository, EmbeddingRepositoryError, EmbeddingStats,
};
use crate::services::embedding_service::{EmbeddingError, EmbeddingService};
use crate::services::search_index_manager::{
    IndexManagerConfig, IndexPriority, SearchIndexManager, SearchIndexManagerImpl,
};
use crate::services::semantic_search_service::{
    EnhancedSearchResult, SearchIndexStats, SemanticSearchError, SemanticSearchService,
};
...
impl EmbeddingRepository for MockEmbeddingRepository {
    ...
    async fn get_embedding_stats(
        &self,
        _project_id: Option<&str>,
    ) -> Result<EmbeddingStats, EmbeddingRepositoryError> {
        let embeddings = self.embeddings.lock().await;
        Ok(EmbeddingStats {
...
fn create_test_context_with_quality(
    id: &str,
    title: &str,
    quality_score: f32,
) -> EnhancedContextItem {
...
fn create_test_context_with_time(
    id: &str,
    title: &str,
...
mod tests {
    ...
    async fn test_auto_index_context_success() {
        ...
        println!(
            "Debug: index_count={}, update_count={}",
            index_count, update_count
        );
    ...
    async fn test_auto_index_context_already_current() {
        ...
        manager
            .set_content_hash_for_testing(&context.id, hash)
            .await;
    ...
    async fn test_queue_priority_ordering() {
        ...
        manager
            .queue_for_indexing(context1, IndexPriority::Low)
            .await
            .unwrap();
        manager
            ...
            .unwrap();
        manager
            .queue_for_indexing(context3, IndexPriority::Critical)
            .await
            .unwrap();
    ...
    async fn test_process_pending_operations_with_failures() {
        ...
        // Queue operation
        manager
            .queue_for_indexing(context, IndexPriority::Normal)
            .await
            .unwrap();
    ...
    async fn test_rebuild_project_index() {
        ...
        let result = manager
            .rebuild_project_index("test-project", &contexts)
            .await;
        assert!(result.is_ok());
    ...
    async fn test_cleanup_stale_embeddings() {
        ...
        let cleaned_count = manager
            .cleanup_stale_embeddings(Some("test-project"))
            .await
            .unwrap();
    ...
    async fn test_batch_processing_performance() {
        ...
        for i in 0..100 {
            contexts.push(create_test_context(
                &format!("test-{}", i),
                &format!("Context {}", i),
            ));
        }
