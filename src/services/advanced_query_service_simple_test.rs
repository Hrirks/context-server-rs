use super::advanced_query_service::{
    AdvancedQueryConfig, AdvancedQueryError, AdvancedQueryService, AdvancedQueryServiceImpl,
    AdvancedSearchFilters, AdvancedSearchQuery, AdvancedSearchResult, QualityRange,
    QuerySuggestion, RankingPreferences, SuggestionType, UserContext, UserPreferences,
};
use tokio;
...
async fn test_advanced_query_service_basic_functionality() {
    ...
    let suggestions = service
        .suggest_queries("authentication", Some("project-1"), &[])
        .await
        .unwrap();
...
async fn test_suggestion_types() {
    ...
    let suggestions = service
        .suggest_queries("how to implement authentication", Some("project-1"), &[])
        .await
        .unwrap();
    ...
    let auth_suggestions: Vec<_> = suggestions
        .iter()
...
async fn test_filter_suggestions() {
    ...
    let quality_suggestions: Vec<_> = filter_suggestions
        .iter()
