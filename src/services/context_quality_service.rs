...
pub trait ContextQualityService: Send + Sync {
    ...
    async fn calculate_quality_metrics(
        ...
        related_contexts: &[EnhancedContextItem],
    ...
    ) -> Result<Vec<QualityImprovement>>;

    /// Batch quality assessment for multiple contexts
    async fn assess_context_batch(
        &self,
        contexts: &[EnhancedContextItem],
    ) -> Result<Vec<ContextQualityAssessment>>;
...
impl ContextQualityService for DefaultContextQualityService {
    async fn calculate_quality_metrics(
        ...
        for analyzer in &self.quality_analyzers {
            analyzer
                .analyze(context, related_contexts, &mut metrics)
                .await?;
        }
    ...
    async fn generate_improvement_suggestions(
        ...
        for generator in &self.improvement_generators {
            let generated = generator
                .generate_suggestions(context, quality_metrics)
                .await?;
...
impl ValidationRule {
    pub fn new(
        ...
        description: &str,
        rule_type: RuleType,
        validator: fn(&EnhancedContextItem) -> bool,
    ) -> Self {
        Self {
            name: name.to_string(),
...
impl QualityAnalyzer for CompletenessAnalyzer {
    async fn analyze(
        &self,
        context: &EnhancedContextItem,
        _related_contexts: &[EnhancedContextItem],
        metrics: &mut QualityMetrics,
    ) -> Result<()> {
        let mut score = 0.0;
        ...
        let type_specific_score = match context.content.content_type {
            ContextType::BusinessRule => {
                ...
                if has_domain && has_pattern {
                    1.0
                } else if has_domain || has_pattern {
                    0.5
                } else {
                    0.0
                }
            ...
            ContextType::ArchitecturalDecision => {
                ...
                if has_decision && has_consequences {
                    1.0
...
impl QualityAnalyzer for ConsistencyAnalyzer {
    async fn analyze(
        ...
        let conflicting_relationships = context
            .relationships
            .iter()
        ...
        if conflicting_relationships > 0 {
            ...
            metrics.issues.push(QualityIssue {
                ...
                description: format!(
                    "Context has {} conflicting relationships",
        ...
        let similar_contexts = related_contexts
            .iter()
            .filter(|other| other.id != context.id && other.project_id == context.project_id)
            .filter(|other| {
                let title_similarity =
                    calculate_similarity(&context.content.title, &other.content.title);
                let desc_similarity =
                    calculate_similarity(&context.content.description, &other.content.description);
        ...
        if metrics.completeness_score < 0.7 {
            if context.content.description.len() < 20 {
                suggestions.push(QualityImprovement {
                    ...
                    description:
                        "Add a more detailed description explaining the context and its purpose"
                            .to_string(),
                    priority: Priority::High,
                    estimated_impact: 0.3,
                    action_required:
                        "Expand the description to at least 20 characters with meaningful content"
                            .to_string(),
                });
            }
            ...
            if context.metadata.tags.is_empty() {
                suggestions.push(QualityImprovement {
                    ...
                    action_required:
                        "Add 2-5 relevant tags that describe the context domain and purpose"
                            .to_string(),
                });
            }
...
impl ImprovementGenerator for ConsistencyImprover {
    async fn generate_suggestions(
        ...
        if metrics.consistency_score < 0.7 {
            let has_conflicts = metrics
                .issues
                .iter()
                .any(|i| i.issue_type == QualityIssueType::ConflictingRules);
            let has_duplicates = metrics
                .issues
                .iter()
                .any(|i| i.issue_type == QualityIssueType::DuplicateContent);

            if has_conflicts {
                suggestions.push(QualityImprovement {
                    suggestion_type: ImprovementType::ResolveConflict,
                    description: "Resolve conflicting relationships with other contexts"
                        .to_string(),
                    priority: Priority::High,
                    estimated_impact: 0.4,
                    action_required: "Review conflicting contexts and resolve inconsistencies"
                        .to_string(),
                });
            }
            ...
            if has_duplicates {
                suggestions.push(QualityImprovement {
                    ...
                    estimated_impact: 0.3,
                    action_required:
                        "Review similar contexts and consolidate or differentiate them".to_string(),
                });
            }
        }
    ...
    async fn generate_suggestions(
        ...
        if context.relationships.is_empty() {
            suggestions.push(QualityImprovement {
                ...
                action_required: "Identify and link related contexts to improve discoverability"
                    .to_string(),
            });
        }
...
mod tests {
    ...
    async fn test_quality_metrics_calculation() {
        ...
        let metrics = service
            .calculate_quality_metrics(&context, &[])
            .await
            .unwrap();
    ...
    async fn test_improvement_suggestions() {
        ...
        let context =
            create_test_context("1", "project1", ContextType::BusinessRule, "Rule", "Short");
        ...
        let suggestions = service
            .generate_improvement_suggestions(&context, &metrics)
            .await
            .unwrap();
        ...
        assert!(suggestions
            .iter()
            .any(|s| s.suggestion_type == ImprovementType::ImproveDescription));
    }
    ...
    async fn test_batch_assessment() {
        ...
        assert!(assessments
            .iter()
            .all(|a| a.quality_metrics.overall_score >= 0.0));
    }
