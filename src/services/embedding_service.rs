...
pub trait EmbeddingService: Send + Sync {
    ...
    fn calculate_similarity(
        &self,
        embedding1: &ContextEmbedding,
        embedding2: &ContextEmbedding,
    ) -> f32;
    ...
    ) -> Result<Vec<VectorSearchResult>, EmbeddingError>;
...
impl EmbeddingService for SimpleEmbeddingService {
    async fn generate_embedding(
        ...
        let preprocessing_steps =
            vec!["text_normalization".to_string(), "tokenization".to_string()];
        ...
        debug!(
            ...
            embedding.embedding_vector.len()
        );
        Ok(embedding)
    }
    ...
    async fn find_similar(
        ...
        debug!(
            "Finding similar embeddings for query, checking {} candidates",
            embeddings.len()
        );
        ...
        for embedding in embeddings {
            ...
            let similarity =
                self.calculate_cosine_similarity(&query_embedding, &embedding.embedding_vector);
...
impl EmbeddingServiceFactory {
    ...
    pub async fn create_initialized_service(
        config: EmbeddingConfig,
    ) -> Result<Box<dyn EmbeddingService>, EmbeddingError> {
