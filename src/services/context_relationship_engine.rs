...
pub trait ContextRelationshipEngine: Send + Sync {
    ...
    async fn detect_relationships(
        ...
        existing_contexts: &[EnhancedContextItem],
    ...
    async fn calculate_relationship_strength(
        &self,
        source: &EnhancedContextItem,
        target: &EnhancedContextItem,
    ) -> Result<f64>;
    ...
    async fn find_related_contexts(
        ...
        contexts: &[EnhancedContextItem],
        max_depth: usize,
    ) -> Result<Vec<RelatedContext>>;

    /// Update relationships when context is modified
    async fn update_relationships_on_change(
        &self,
        changed_context: &EnhancedContextItem,
        all_contexts: &[EnhancedContextItem],
    ) -> Result<Vec<RelationshipUpdate>>;
}
...
impl ContextRelationshipEngine for DefaultContextRelationshipEngine {
        ...
        for (i, source) in contexts.iter().enumerate() {
            ...
            let relationships = self
                .detect_relationships(
                    source,
                    &remaining_contexts.into_iter().cloned().collect::<Vec<_>>(),
                )
                .await?;
    ...
    async fn update_relationships_on_change(
        ...
        let new_relationships = self
            .detect_relationships(changed_context, all_contexts)
            .await?;
        ...
        for existing_rel in &changed_context.relationships {
            if let Some(target_context) =
                all_contexts.iter().find(|c| c.id == existing_rel.target_id)
            {
                let new_strength = self
                    .calculate_relationship_strength(changed_context, target_context)
                    .await?;
...
impl KeywordExtractor for TechnicalTermExtractor {
    fn extract_keywords(&self, context: &EnhancedContextItem) -> Vec<String> {
        ...
        let technical_pattern =
            regex::Regex::new(r"[A-Z][a-z]+(?:[A-Z][a-z]+)*|[a-z]+_[a-z_]+").unwrap();
...
impl RelationshipDetector for ContentSimilarityDetector {
    fn detect_relationship(
        ...
        let similarity =
            calculate_text_similarity(&source.content.description, &target.content.description);
...
impl RelationshipDetector for TypeBasedDetector {
    fn detect_relationship(
        ...
        let relationship_type = match (&source.content.content_type, &target.content.content_type) {
            (ContextType::BusinessRule, ContextType::ArchitecturalDecision) => {
                Some(RelationshipType::Constrains)
            }
            (ContextType::ArchitecturalDecision, ContextType::BusinessRule) => {
            ...
            (ContextType::FeatureContext, ContextType::BusinessRule) => {
                Some(RelationshipType::Implements)
            }
            (ContextType::BusinessRule, ContextType::FeatureContext) => {
                Some(RelationshipType::Validates)
            }
            _ => None,
        };
        ...
        if let Some(rel_type) = relationship_type {
            Some(
                ContextRelationship::new(target.id.clone(), rel_type, 0.7, true)
                    .with_confidence(0.8),
            )
        } else {
            None
...
mod tests {
    ...
    async fn test_relationship_detection() {
        ...
        let relationships = engine
            .detect_relationships(&context1, &existing_contexts)
            .await
            .unwrap();
    ...
    async fn test_find_related_contexts() {
        ...
        let related = engine
            .find_related_contexts(&"1".to_string(), &contexts, 2)
            .await
            .unwrap();
        ...
        assert!(related
            .iter()
            .any(|r| r.context.id == "2" || r.context.id == "3"));
    }
    ...
    fn test_text_similarity() {
        ...
        let identical_similarity = calculate_text_similarity("test text", "test text");
        assert_eq!(identical_similarity, 1.0);

        // Test completely different texts
        let no_similarity =
            calculate_text_similarity("completely different", "nothing matches here");
